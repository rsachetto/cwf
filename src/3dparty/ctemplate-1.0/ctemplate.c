...
static template *newtemplate(const char *filename, const char *tmplstr, const TMPL_fmtlist *fmtlist, sds *out_string, FILE *out, FILE *errout) {
    template *t;
    FILE *fp;
    char *buf = 0;
    struct stat stb;
    ...
    if(tmplstr == 0) {
        if((fp = fopen(filename, "r")) != 0 && fstat(fileno(fp), &stb) == 0 && S_ISREG(stb.st_mode) != 0 && (buf = (char *)mymalloc(stb.st_size + 1)) != 0 &&
...
static tagnode *newtag(template *t, tag_kind kind) {
    tagnode *ret;
...
static void freetag(tagnode *tag) {
    template *t;
...
static const char *tagname(tag_kind kind) {
    switch(kind) {
    case tag_var:
        return "TMPL_VAR";
    case tag_if:
...
static TMPL_fmtfunc findfmt(const TMPL_fmtlist *fmtlist, const char *name) {
...
static tagnode *scantag(template *t, const char *p) {
    tag_kind kind;
    ...
    case tag_var:
        while((name == 0 && (name = scanattr(t, "name", p)) != 0) || (fmt == 0 && (fmt = scanattr(t, "fmt", p)) != 0) ||
    ...
    case tag_if:
    case tag_elsif:
        while((name == 0 && (name = scanattr(t, "name", p)) != 0) || (value == 0 && (value = scanattr(t, "value", p)) != 0)) {
            p = scanspaces(t, t->scanptr);
        }
        break;
    ...
    case tag_break:
    case tag_cont:
        if(p != t->scanptr && (value = scanattr(t, "level", p)) != 0) {
            p = scanspaces(t, t->scanptr);
        }
        break;
    }
    ...
    if(commentish == 0 && p[0] == '>') {
        t->scanptr = p + 1;
    } else if(commentish == 0 && container == 0 && p[0] == '/' && p[1] == '>') {
        t->scanptr = p + 2;
    } else if(commentish != 0 && p[0] == '-' && p[1] == '-' && p[2] == '>') {
        t->scanptr = p + 3;
    } else {
        goto failure;
    }
...
static void parseif(template *t, int stop) {
...
static void parseloop(template *t, int stop) {
    ...
    if(t->curtag != 0 && t->curtag->kind == tag_endloop) {
        ...
        if(t->errout != 0) {
            fprintf(t->errout,
                    "TMPL_LOOP tag in file \"%s\" line %d "
...
static tagnode *parselist(template *t, int stop) {
    ...
    while((tag = t->curtag) != 0) {
        ...
        case tag_endloop:
            ...
            if(t->errout != 0) {
                fprintf(t->errout,
                        "Unexpected %s tag in file \"%s\" "
                        "line %d\n",
                        tagname(tag->kind), t->filename, t->tagline);
            }
            t->error = 1;
            scan(t);
...
static char *valueof(const char *varname, const TMPL_varlist *varlist) {
    TMPL_var *var;
...
static TMPL_loop *findloop(const char *loopname, const TMPL_varlist *varlist) {
    TMPL_loop *loop;
...
static int is_true(const tagnode *iftag, const TMPL_varlist *varlist) {
    ...
    return (testval == 0 && value != 0 && *value != 0) ||

           (testval == 0 && loop != 0) ||

           (testval != 0 && *testval == 0 && loop == 0 && (value == 0 || *value == 0)) ||
...
static void write_text(const char *p, int len, sds *out_string, FILE *out) {
    int i, k;
...
static const char *newfilename(const char *inclfile, const char *parentfile) {
    char *newfile, *cp;
...
static void walk(template *t, tagnode *tag, const TMPL_varlist *varlist) {
    const char *value;
    TMPL_loop *loop;
    TMPL_varlist *vl;
    template *t2;
    ...
    if(tag == 0 || t->break_level > 0 || t->cont_level > 0 || t->error != 0) {
        return;
    }
    switch(tag->kind) {
    ...
    case tag_var:
        if((value = valueof(tag->tag.var.varname, varlist)) == 0 && (value = tag->tag.var.dfltval) == 0) {
            break;
        }
...
TMPL_varlist *TMPL_add_var(TMPL_varlist *varlist, ...) {
    ...
    va_start(ap, varlist);
    while((name = va_arg(ap, char *)) != 0 && (value = va_arg(ap, char *)) != 0) {
        /*
...
TMPL_varlist *TMPL_add_int_var(TMPL_varlist *varlist, char *name, int value) {
    ...
    char num_char[11];
    sprintf(num_char, "%d", value);

    return TMPL_add_var(varlist, name, num_char, 0);
}

TMPL_varlist *TMPL_add_float_var(TMPL_varlist *varlist, char *name, float value) {

    char num_char[256];
    sprintf(num_char, "%f", value);
...
TMPL_varlist *TMPL_add_loop(TMPL_varlist *varlist, const char *name, TMPL_loop *loop) {
    ...
    for(lp = varlist->parent; lp != 0; lp = lp->parent == 0 ? 0 : lp->parent->parent) {
        if(lp == loop) {
            return varlist;
        }
    }
...
TMPL_loop *TMPL_add_varlist(TMPL_loop *loop, TMPL_varlist *varlist) {
    ...
    for(vl = loop->parent; vl != 0; vl = vl->parent == 0 ? 0 : vl->parent->parent) {
        if(vl == varlist) {
            return loop;
        }
    }
